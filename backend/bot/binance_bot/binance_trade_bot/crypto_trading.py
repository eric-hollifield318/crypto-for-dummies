...
def main():
    ...
    crypto_trading_bot = [t for t in threading.enumerate() if t.name == "crypto_for_dummies_bot_thread"][0]
    while True and not crypto_trading_bot.stopped():
        schedule.run_pending()
        time.sleep(1)
    
    if crypto_trading_bot.stopped():
